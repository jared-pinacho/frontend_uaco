...
export const FormularioConsejeros = ({
  actualizarTabla,
  modo,
  consejeroAEditar,
}) => {
  ...
  useEffect(() => {
    ...
    if ((modo === "editar" || modo === "eliminar") && consejeroAEditar) {
      obtenerColonias(consejeroAEditar.direccion.colonia.cp.id_cp);
      setFormData({
        matricula: consejeroAEditar.matricula || "",
        nombre: consejeroAEditar.nombre || "",
        apellidopaterno: consejeroAEditar.apellido_paterno || "",
        apellidomaterno: consejeroAEditar.apellido_materno || "",
        curp: consejeroAEditar.curp || "",
        sexo: consejeroAEditar.sexo || "",
        fecha_nacimiento: consejeroAEditar.fecha_nacimiento || "",
        rfc: consejeroAEditar.rfc || "",
        niveleducativo: consejeroAEditar.nivel_educativo || "",
        clavecuc: consejeroAEditar.clave_cuc || "",
        email: consejeroAEditar.usuario.email || "",
        telefono: consejeroAEditar.telefono || "",
        calle: consejeroAEditar.direccion.calle || "",
        num_exterior: consejeroAEditar.direccion.num_exterior || "",
        colonia: consejeroAEditar.direccion.colonia.id_colonia || "",
        municipio: consejeroAEditar.direccion.colonia.municipio.nombre || "",
        cp: consejeroAEditar.direccion.colonia.cp.id_cp || "",
        estado:
          consejeroAEditar.direccion.colonia.municipio.estado.nombre || "",
        id_tiposangre: consejeroAEditar.id_tiposangre || "",
        telefono_emergencia: consejeroAEditar.telefono_emergencia || "",
        padecimiento: consejeroAEditar.padecimiento || "",
        nacionalidad: consejeroAEditar.id_nacionalidad || "",
        estado_nacimiento: consejeroAEditar.estado_nacimiento || "",
        perfil_academico: consejeroAEditar.perfil_academico || "",
        tipos_consejero: consejeroAEditar.tipos_consejeros || [],
      });
    } else {
  ...
  const handleCheckboxChange = (updatedTipos) => {
    setFormData({
      ...formData,
      tipos_consejero: updatedTipos,
    });
  ...
  const validateForm = (formData) => {
    ...
    if (formData.tipos_consejero.includes("1")) {
      // console.log("entre validacion correo");
      ...
      if (modo === "agregar") {
        if (!formData.password.trim()) {
          ...
          errors.password =
            "La 'Contraseña' debe contener de 8 a 12 caracteres";
  ...
  const handleSubmit = (event) => {
    ...
    if (modo === "agregar") {
      ...
      // console.log(formData);
      axios
        .post(`${apiUrl}consejeros`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      ...
      // console.log(formData);
      axios
        .put(`${apiUrl}consejeros/${consejeroAEditar.matricula}`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      ...
      axios
        .delete(`${apiUrl}consejeros/${consejeroAEditar.matricula}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
        ...
        {modo === "agregar" ? (
          <div className="opcionesDias">
            <CheckBoxList
              ...
              datoform={"tipos_consejero"}
            />
          </div>
        ) : null}

        {formData.tipos_consejero.includes("1") ? (
          <>
            <div>
              <label>Correo:</label>
              <input

...
export const FaseDosEstudiante = ({informacion, actualizar,setActualizar}) => {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      reporte_uno:informacion.faseDos?.reporte_uno || "",
...
const cambiarInforme1 = (dato) => {
  axios
    .patch(
      `${apiUrl}estado/cambio/presentacion/informe1/${dato}`,
      {},
      {
        headers: {
...

{
  table: {
    ...
    [
      { text: 'Mes 1', style: 'tabla' },
      { text: `${formData.actividad}`, style: 'tabla' }
  ], // Primera fila
  [
    { text: 'Mes 2', style: 'tabla' },
    { text: `${formData.actividad}`, style: 'tabla' }
], // Primera fila
    ],
    heights: [40, 50, 50],
...
{ text:  `\nObservaciones:________________________________________________________________________________________________________________________________________________________________`, style: 'obser' },
  ...
  const handleSubmitInforme1 = async () => {
    ...
    if (selectedFile) {
        ...
        axios
        .post(`${apiUrl}subir/doc/informe1`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
...
<button className="subir-3z" onClick={handleSubmitInforme1}>Subir</button>
...
<button className="ver-2z" onClick={() => descargarArchivo(formData.reporte_uno)} >Ver</button>

<p className="arc-2z" >Archivo subido: {formData.reporte_uno}</p>
...
<div className="accionesz-1">
       ...
       <button className="verz-1" onClick={() => 
        ...
        descargarArchivo(formData.reporte_uno)}
...
<button className="verz-3" onClick={() => descargarArchivo(formData.reporte_uno)} >Ver</button>
<p className="arc-3z" >Archivo subido: {formData.reporte_uno}</p>
...
<label className="esz-3">Estado: Revisado no aprobado </label>
           ...
           <span >{formData.comentario_r1}</span>

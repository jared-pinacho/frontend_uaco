...
import { VentanaInforme1 } from "../VentanaRevisar/VentanaInforme1";



export const FaseDosEscolar = ({informacion,actualizar,setActualizar}) => {
  ...
  useEffect(() => {
    if (informacion) {
      setFormData({
        reporte: informacion.faseDos?.reporte_uno || "",
        estado: informacion.faseDos?.estatus_envio || "",
        comentario: informacion.faseDos?. comentario || "",
      });
      ...
      if(informacion.faseDos ===null){
        ...
        setVisible2(informacion.faseDos?.estatus_envio);
      }
...
const cambiarEstadoInforme = (estado) => {
  axios
    .patch(
      `${apiUrl}cambio/estado/informe1/${informacion.estudiante.matricula}/${estado}`,
      {},
      {
        headers: {
...
const descargarArchivo = (nombreArchivo) => {
  ...
  return (
    <div className="Festudian">
      <label className="titul">Primer Reporte Bimestral</label>
      
      <div className="contenedor">
        <label className="titulo-contenedor">Reporte bimestral 1</label>
...
{mostrarComponente2 && <VentanaInforme1

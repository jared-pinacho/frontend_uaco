...
import "../FormularioFacilitadores/FormularioFacilitadores.css";
import { toast } from "react-toastify";
import Cookies from "js-cookie";
import { URL_API } from "../../Services/Const";
import { SelectInput } from "../SelecInput/SelectInput";
export const FormularioFacilitadores = ({
  actualizarTabla,
  modo,
  facilitadorAEditar,
}) => {
  ...
  useEffect(() => {
    ...
    if ((modo === "editar" || modo === "eliminar") && facilitadorAEditar) {
      obtenerColonias(facilitadorAEditar.direccion.colonia.cp.id_cp);
      setFormData({
        matricula: facilitadorAEditar.matricula || "",
        nombre: facilitadorAEditar.nombre || "",
        apellidopaterno: facilitadorAEditar.apellido_paterno || "",
        apellidomaterno: facilitadorAEditar.apellido_materno || "",
        curp: facilitadorAEditar.curp || "",
        sexo: facilitadorAEditar.sexo || "",
        fecha_nacimiento: facilitadorAEditar.fecha_nacimiento || "",
        rfc: facilitadorAEditar.rfc || "",
        niveleducativo: facilitadorAEditar.nivel_educativo || "",
        email: facilitadorAEditar.usuario.email || "",
        telefono: facilitadorAEditar.telefono || "",
        calle: facilitadorAEditar.direccion.calle || "",
        num_exterior: facilitadorAEditar.direccion.num_exterior || "",
        colonia: facilitadorAEditar.direccion.colonia.id_colonia || "",
        municipio: facilitadorAEditar.direccion.colonia.municipio.nombre || "",
        cp: facilitadorAEditar.direccion.colonia.cp.id_cp || "",
        estado:
          facilitadorAEditar.direccion.colonia.municipio.estado.nombre || "",
        id_tiposangre: facilitadorAEditar.id_tiposangre || "",
        telefono_emergencia: facilitadorAEditar.telefono_emergencia || "",
        padecimiento: facilitadorAEditar.padecimiento || "",
        nacionalidad: facilitadorAEditar.id_nacionalidad || "",
        estado_nacimiento: facilitadorAEditar.estado_nacimiento || "",
        perfil_academico: facilitadorAEditar.perfil_academico || "",
      });
    } else {
  ...
  const validateForm = (formData) => {
    ...
    if (!formData.email.trim()) {
      errors.email = "El 'correo del facilitador' es requerida";
    } else if (!regexEmail.test(formData.email.trim())) {
      errors.email = "Por favor ingrese un correo valido";
    }
    if (modo === "agregar") {
      if (!formData.password.trim()) {
        errors.password = "La 'Contraseña del facilitador' es requerido";
      } else if (!regexPasswordSize.test(formData.password.trim())) {
        errors.password =
          "La 'Contraseña del facilitador' debe contener de 8 a 12 caracteres";
      } else if (!regexPassword.test(formData.password.trim())) {
        errors.password =
          "La 'Contraseña del facilitador' debe contener almenos un número y una letra";
      }
    }

    if (!formData.matricula.trim()) {
      errors.matricula = "La 'matricula del facilitador' es requerida";
    } else if (!regexMatricula.test(formData.matricula.trim())) {
      errors.matricula =
        "La 'matricula del facilitador' debe contener 11 caracteres";
    }

    if (!formData.nombre.trim()) {
      errors.nombre = "El 'nombre del facilitador' es requerido";
    } else if (!regexName.test(formData.nombre.trim())) {
      errors.nombre = "El 'Nombre del facilitador' debe contener solo letras";
    }

    if (!formData.apellidopaterno.trim()) {
      errors.apellidopaterno =
        "El 'apellido paterno del facilitador' es requerido";
    } else if (!regexName.test(formData.apellidopaterno.trim())) {
      errors.apellidopaterno =
        "El 'apellido paterno del facilitador' debe contener solo letras";
    }

    if (!formData.apellidomaterno.trim()) {
      errors.apellidomaterno =
        "El 'apellido materno del facilitador' es requerido";
    } else if (!regexName.test(formData.apellidomaterno.trim())) {
      errors.apellidomaterno =
        "El 'apellido materno del facilitador' debe contener solo letras";
    }
    ...
    if (!formData.rfc.trim()) {
      errors.rfc = "El 'RFC del facilitador' es requerido";
    } else if (!regexRfcPersonaFisica.test(formData.rfc.trim())) {
      errors.rfc = "El 'RFC del facilitador' no es válida";
    }

    if (!formData.niveleducativo.trim()) {
      errors.niveleducativo =
        "La 'nivel educativo del facilitador' es requerido";
    } else if (!regexName.test(formData.niveleducativo.trim())) {
      errors.niveleducativo =
        "El 'nivel educativo del facilitador' debe contener solo letras";
    }
    ...
    if (String(formData.nacionalidad) === "2") {
      if (!formData.curp.trim()) {
        ...
        if (
          ...
          const inicialyconsonanteApellidoP = `${apellido_paterno.charAt(0)}${apellido_paterno.match(/[aeiou]/i)?.[0] || ""
            }`.toUpperCase();
  ...
  const handleSubmit = (event) => {
    ...
    if (modo === "agregar") {
      ...
      axios
        .post(`${apiUrl}facilitadores`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      ...
      axios
        .put(
          `${apiUrl}facilitadores/${facilitadorAEditar.matricula}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      axios
        .delete(`${apiUrl}facilitadores/${facilitadorAEditar.matricula}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
